...
### Requirement: ServiceExplorer Island with Grupo Tabs and Per-Grupo CTA
...
### Requirement: WhyUs Three Domain Proof Points
...
(Previously: fixed 48px/56px, zero CTAs, sentinel coupled)
...
(Previously: fixed px tokens, flat divider)
...
`#abogados` MUST show Omar `Especialista en Derecho Laboral y Seguridad Social` and Franco `Especialista en Derecho Penal y Procesal Penal` verbatim §2, bios only, no duplication. Each MUST show chips to `#servicios` anchors. MUST NOT cross-attribute or invent areas.
(Previously: Team duplicated service cards)
...
(Previously: placeholder map, tel not prominent)
...
(Previously: 4 anchors, no Servicios)
...
### Requirement: Surface Scale Application
(Reason: subsumed by fluid @theme + dossier rule)
(Migration: verify `surface-container-*` via Design Tokens scenario)
